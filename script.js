...
// heart-increases function
const heartButtons = document.getElementsByClassName("heart-click");
for (let i = 0; i < heartButtons.length; i++) {
  heartButtons[i].addEventListener('click', function (e) {
    e.preventDefault();
    const lifeline = parseInt(document.getElementById("heart-increases").innerText);
    const incresesheart = lifeline + 1;
    document.getElementById("heart-increases").innerText = incresesheart;
  });
}
...
const handelClick = () => {
  // console.log('clicked');
  const DeleteHistory = document.getElementById('transaction-container')

  DeleteHistory.innerHTML = "";

}

// callHistory Functionality
...
for (let i = 0; i < copyButton.length; i++) {
  copyButton[i].addEventListener('click', function (e) {
    ...
    e.preventDefault();
    const copies = parseInt(document.getElementById("copyBtn").innerText);
    const copyincreses = copies + 1;
    document.getElementById("copyBtn").innerText = copyincreses;
  });
